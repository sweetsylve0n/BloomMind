    ...
    suspend fun sendMessage(userMessage: String): Result<String> {
        try {
            val request = GeminiRequestDto(
                contents = listOf(
                    GeminiContent(parts = listOf(GeminiPart(text = "Responde a lo siguiente de forma concisa, amable, con empatia por el usuario, en un solo párrafo y haciendo una pregunta al final para continuar la conversacion: $userMessage")))
                ),
            ...
            // Añadimos más detalle al error de conexión
